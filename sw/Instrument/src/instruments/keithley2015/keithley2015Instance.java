/***********************************************************
 * Software: instrument client
 * Module:   Keithley 2015 instrument instances managing class
 * Version:  0.1
 * Licence:  GPL2
 *
 * Owner: Kim Kristo
 * Date creation : 1.4.2013
 *
 ***********************************************************/
package instruments.keithley2015;

import components.InstanceBase;
import components.XmlReader;
import components.XmlWriter;
import interfaces.*;
import org.apache.log4j.Logger;
import instruments.*;
import java.util.Hashtable;
import javax.swing.ImageIcon;
import murlen.util.fscript.FSExtension;

public class keithley2015Instance extends InstanceBase {

    private MessageInterface msg;

    /** Creates new form HP3480Instance */
    public keithley2015Instance(String name, Hashtable<String, String> properties,
            DesktopInterface desktop, MessageInterface msg) {
        // get logger instance for this class
        logger = Logger.getLogger(keithley2015Instance.class);

        this.desktop = desktop;
        this.msg = msg;
        this.name = name;

        initComponents();

        // set default icon
        java.net.URL imageURL = this.getClass().getResource("/resources/64x64_multimeter.png");

        // load icon image
        if (imageURL != null) {
            image = new ImageIcon(imageURL);
            this.setIcon(image);
        }

        // create measurement device ui
        device = new keithley2015(name, properties, msg);
        // create command interface for scripting
        scriptExtension = new Keithley2015Extension((commandExecutionInterface) device);
    }

    @Override
    public FSExtension getScriptExtension() {
        return (scriptExtension);
    }

    /** Function implements custom xml handler
     * 
     * @param r instance of XmlReader
     */
    @Override
    public void loadConfig(XmlReader r) {
        // load scope settings
        ((keithley2015) device).loadSettings(r);
    }

    /** Function implements prototype of custom xml writer
     * 
     * @param w instance of XmlWriter
     */
    @Override
    public void saveConfig(XmlWriter w) {
        // save scope settings
        ((keithley2015) device).saveSettings(w);
    }

    /** This method is called from within the constructor to
     * initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is
     * always regenerated by the Form Editor.
     */
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        setBorder(new javax.swing.border.SoftBevelBorder(javax.swing.border.BevelBorder.RAISED));
        setToolTipText("Keithley 2015 Multimeter");
        setPreferredSize(new java.awt.Dimension(64, 80));
        addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                formMouseClicked(evt);
            }
        });
        setLayout(new java.awt.BorderLayout());
    }// </editor-fold>//GEN-END:initComponents
    private void formMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseClicked
        mouseClicked(evt);
    }//GEN-LAST:event_formMouseClicked
    // Variables declaration - do not modify//GEN-BEGIN:variables
    // End of variables declaration//GEN-END:variables
}
